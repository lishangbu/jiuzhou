...
/** 启动挂机会话 */
export const startIdleSession = (params: IdleStartParams): Promise<IdleStartResponse> =>
  api.post('/idle/start', params);

/** 停止挂机会话 */
export const stopIdleSession = (): Promise<IdleApiResponse> =>
  api.post('/idle/stop');
...
export const getIdleHistory = (): Promise<IdleHistoryResponse> =>
  api.get('/idle/history');

/** 查询会话内战斗批次（回放） */
export const getIdleBatches = (sessionId: string): Promise<IdleBatchesResponse> =>
  api.get(`/idle/history/${sessionId}/batches`);

/** 标记会话已查看 */
export const markIdleSessionViewed = (sessionId: string): Promise<IdleApiResponse> =>
  api.post(`/idle/history/${sessionId}/viewed`);

/** 断线补全（活跃会话 + 最新批次） */
export const getIdleProgress = (): Promise<IdleProgressResponse> =>
  api.get('/idle/progress');
...
/** 更新挂机配置 */
export const updateIdleConfig = (config: Omit<IdleConfigDto, 'mapId' | 'roomId'> & {
  mapId?: string | null;
  roomId?: string | null;
}): Promise<IdleApiResponse> =>
  api.put('/idle/config', config);
