 ...
 *
 * 作用（做什么 / 不做什么）：
 * 1. 做什么：集中处理一字焚诀输入的前端裁剪、长度统计、请求值转换与提示文案，避免 `TechniqueModal` 和 `ResearchPanel` 各写一套。
 * 2. 做什么：为输入框和任务结果展示提供统一文案入口，让“留空随机、最多 1 个中文字符”的交互语义只维护一次。
 * 3. 不做什么：不直接发请求、不持有 React 状态，也不替代服务端最终校验。
 *
 * 输入/输出：
 * - 输入：玩家在输入框中的原始文本，或任务状态里的已选一字焚诀。
 ...
 * 2. 输入与任务回显必须共用同一套标签文案，否则玩家会看到“提交时叫一字焚诀、结果里又换了名字”的语义分叉。
 */
