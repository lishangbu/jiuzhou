 ...
 * 2. 做什么：覆盖 `/assets/*` 走 CDN、`/uploads/*` 走源站两类核心分支，避免以后再把入口资源写回源站绝对地址。
 * 3. 不做什么：不依赖真实浏览器 location，不验证 axios，也不跑页面渲染。
 *
 * 输入/输出：
 * - 输入：资源路径字符串与人工构造的 `{ serverBase, cdnBase }`。
 * - 输出：`buildAssetUrl` 产出的最终访问 URL。
 *
 * 数据流/状态流：
 * - 业务或入口层提供资源相对路径
 * - `buildAssetUrl` 依据统一 host 配置拼接
 * - 返回值被 favicon 注入与图片展示逻辑共同消费
 *
 * 关键边界条件与坑点：
 * 1. `/assets/favicon.png` 必须命中 `cdnBase`，这是这次线上 favicon 走错域名的根因位置。
 * 2. `/uploads/*` 不能走 CDN，否则玩家上传头像会直接失效，所以这里单独锁住源站分支。
 */

import { describe, expect, it } from 'vitest';
import { buildAssetUrl } from '../runtimeUrls';
