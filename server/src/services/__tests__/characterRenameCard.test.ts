...
test('renameCharacterWithCard: 应在成功改名时扣除 1 张易名符并刷新角色缓存', async (t) => {
  ...
  const broadcastMock = t.mock.method(
    characterServiceSideEffects,
  ...
  t.mock.method(database.pool, 'connect', async () =>
    createMockPoolClient(async (sql, params) => {
      ...
      if (sql.includes('SELECT id, nickname FROM characters WHERE user_id = $1 LIMIT 1 FOR UPDATE')) {
        return createQueryResult([{ id: 88, nickname: '旧道号' }]);
      }
  ...
  assert.deepEqual(broadcastMock.mock.calls[0]?.arguments, [{
    senderTitle: '天机传音',
    content: '【易名符】旧道号改名为【凌霄子】，仙名重铸，声传九州！',
  }]);
});
...
test('renameCharacterWithCard: 非易名符实例不应被用于改名', async (t) => {
  ...
  t.mock.method(database.pool, 'connect', async () =>
    createMockPoolClient(async (sql) => {
      ...
      if (sql.includes('SELECT id, nickname FROM characters WHERE user_id = $1 LIMIT 1 FOR UPDATE')) {
        return createQueryResult([{ id: 66, nickname: '原道号' }]);
      }
  ...
  assert.equal(invalidateMock.mock.callCount(), 0);
