 ...
 * 2. 这里把“时间预算”和“派发上限”拆成独立断言，后续调参时只需要改常量，不需要重写测试结构。
 *
 * 关键边界条件与坑点：
...
test('shouldContinueOnlineBattleSettlementDispatch: 命中时间预算时应停止补派', () => {
  assert.equal(
    shouldContinueOnlineBattleSettlementDispatch({
      drainAll: false,
      elapsedMs: 1200,
