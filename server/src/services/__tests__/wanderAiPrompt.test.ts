...
test('buildWanderAiEpisodeSetupPromptRuleSet: 应显式包含幕次生成的主题、引子、标题与选项约束', () => {
  ...
  assert.match(ruleSet.outputRules.openingStyleRule, /禁止复述最近一幕 summary 已明确写出的动作/u);
  assert.doesNotMatch(ruleSet.outputRules.openingStyleRule, /带过余波/u);
...
test('buildWanderAiEpisodeSetupSystemMessage: 终幕待选幕次必须禁止提前写尾声与称号', () => {
  ...
  assert.match(systemMessage, /player\.storyOtherPlayer/u);
...
test('buildWanderAiEpisodeSetupUserPayload: 不应再把 mainQuestName 透传给模型，且应暴露终幕待选标记', () => {
  ...
  assert.equal(payload.player.storyOtherPlayer, null);
  assert.equal(payload.storyLocation.fullName, '东洲·青云村外·南侧密林');
});

test('buildWanderAiEpisodeSetupUserPayload: 前文上下文与故事级伙伴和其他玩家快照必须完整透传', () => {
  const payload = buildWanderAiEpisodeSetupUserPayload({
    nickname: '测试角色',
    realm: '炼气期',
    hasTeam: false,
  ...
  assert.equal(payload.story.storySummary, null);
  assert.equal(payload.player.storyPartner?.nickname, '小瓯');
  assert.equal(payload.player.storyPartner?.role, '辅助');
  assert.equal(payload.player.storyOtherPlayer?.nickname, '陆听潮');
  assert.equal(payload.player.storyOtherPlayer?.title, '巡河客');
});
...
test('buildWanderAiEpisodeResolutionPromptRuleSet: 终幕结算必须显式暴露称号颜色与属性白名单约束', () => {
  ...
  assert.match(ruleSet.outputRules.summaryStyleRule, /直接造成的结果、局势变化或收束/u);
  assert.doesNotMatch(ruleSet.outputRules.summaryStyleRule, /余波/u);
  assert.doesNotMatch(ruleSet.outputRules.summaryExample, /余波之中/u);
  assert.doesNotMatch(ruleSet.outputRules.summaryExample, /这一幕落在/u);
...
test('buildWanderAiEpisodeResolutionSystemMessage: 非终幕结算必须强调称号字段清空', () => {
  ...
  assert.match(systemMessage, /player\.storyPartner/u);
  assert.match(systemMessage, /player\.storyOtherPlayer/u);
  assert.doesNotMatch(systemMessage, /余波/u);
...
test('buildWanderAiEpisodeResolutionUserPayload: 应透传本幕选择与终幕结算模式', () => {
  ...
  assert.equal(payload.player.storyOtherPlayer?.nickname, '顾照雪');
  assert.equal(payload.player.storyOtherPlayer?.realm, '结丹期');
});
