...
test('resolveTechniqueResearchFragmentCost: 未启用顿悟符时应返回基础残页消耗', () => {
  assert.equal(TECHNIQUE_RESEARCH_BASE_FRAGMENT_COST, 5_000);
  assert.equal(resolveTechniqueResearchFragmentCost(false), 5_000);
});

test('resolveTechniqueResearchFragmentCost: 启用顿悟符时应返回半价残页消耗', () => {
  assert.equal(TECHNIQUE_RESEARCH_COOLDOWN_BYPASS_FRAGMENT_COST, 2_500);
  assert.equal(resolveTechniqueResearchFragmentCost(true), 2_500);
});
