...
test('itemService 与 partnerService: 应通过统一物品入口消费归元洗髓露并刷新动态伙伴快照', () => {
  ...
  const partnerReboneServiceSource = readSource('../partnerReboneService.ts');
  ...
  assert.match(itemServiceSource, /effectType === 'reroll_partner_base_attrs'[\s\S]*?await partnerReboneService\.createPendingReboneJob\(\{/u);
  assert.match(
  ...
  assert.match(
    partnerReboneServiceSource,
    /async processPendingReboneJob\([\s\S]*?executeGeneratedPartnerRebone\(\{[\s\S]*?refreshGeneratedSnapshots:\s*true,[\s\S]*?includePartnerDetail:\s*false,[\s\S]*?\}\)/u,
  );
});
