 ...
 *   复用 mergeRewardItems（来自 idleSessionService）验证物品合并正确性。
 *
 * 输入/输出：
 ...
 *   2. 物品合并按 itemDefId 累加数量，不重复创建条目
 */
...
function computeSessionSummary(batches: BatchSummary[]): {
  ...
  let winCount = 0;
  let loseCount = 0;
  let rewardItems: RewardItemEntry[] = [];

  for (const batch of batches) {
    // 只有胜利时才累加 exp/silver/items（与 executeSingleBatch 保证一致）
    if (batch.result === 'attacker_win') {
      totalExp += batch.expGained;
      totalSilver += batch.silverGained;
      winCount++;
      rewardItems = mergeRewardItems(rewardItems, batch.itemsGained);
    } else {
      loseCount++;
    }
  }
...
test('属性 11：奖励汇总正确性 — total_exp/total_silver 等于所有胜利 batch 之和（numRuns: 100）', () => {
  ...
  for (let run = 0; run < numRuns; run++) {
    ...
    for (let i = 0; i < batchCount; i++) {
      ...
      batches.push({ result, expGained, silverGained, itemsGained: [] });
...
test('属性 11：物品合并正确性 — 相同 itemDefId 累加数量（numRuns: 100）', () => {
  // Feature: offline-idle-battle, Property 11: 奖励汇总正确性（物品部分）
  // 验证：需求 4.5
  // 属性：mergeRewardItems 对相同 itemDefId 累加数量，不重复创建条目

  const numRuns = 100;
  let failCount = 0;
  const failures: string[] = [];

  const rng = makeLcgRng(77777);

  for (let run = 0; run < numRuns; run++) {
    // 随机生成 2~5 种物品，每种物品随机出现 1~5 次
    const itemTypeCount = Math.floor(rng() * 4) + 2;
    const itemIds = Array.from({ length: itemTypeCount }, (_, i) => `item-${i}`);

    // 构造多个 batch 的物品列表
    const batchCount = Math.floor(rng() * 10) + 2;
    const allBatchItems: RewardItemEntry[][] = [];
    const expectedQuantities = new Map<string, number>();

    for (let b = 0; b < batchCount; b++) {
      const batchItems: RewardItemEntry[] = [];
      for (const itemId of itemIds) {
        if (rng() < 0.5) {
          const qty = Math.floor(rng() * 5) + 1;
          batchItems.push({ itemDefId: itemId, itemName: itemId, quantity: qty });
          expectedQuantities.set(itemId, (expectedQuantities.get(itemId) ?? 0) + qty);
        }
      }
    ...
    // 逐步合并所有 batch 的物品
    let merged: RewardItemEntry[] = [];
    for (const batchItems of allBatchItems) {
      merged = mergeRewardItems(merged, batchItems);
    }

    // 验证合并结果
    for (const [itemId, expectedQty] of expectedQuantities) {
      const found = merged.find((item) => item.itemDefId === itemId);
      if (!found) {
        failCount++;
        if (failures.length < 3) {
          failures.push(`run=${run}: itemId=${itemId} 在合并结果中不存在`);
        }
        continue;
      }
      if (found.quantity !== expectedQty) {
        failCount++;
        if (failures.length < 3) {
          failures.push(
            `run=${run}: itemId=${itemId} 期望数量=${expectedQty}，实际=${found.quantity}`
          );
        }
      }
    }

    // 验证没有多余条目（合并后条目数 ≤ itemTypeCount）
    if (merged.length > itemTypeCount) {
      failCount++;
      if (failures.length < 3) {
        failures.push(
          `run=${run}: 合并后条目数=${merged.length}，超过物品种类数=${itemTypeCount}`
        );
      }
    }
  }
  ...
  assert.equal(
    failCount,
    0,
    `属性 11（物品合并）失败 ${failCount}/${numRuns} 次。前几个失败用例：\n${failures.join('\n')}`
  );
});
...
test('属性 11 边界：空 batch 列表时汇总全为零', () => {
  ...
  assert.equal(summary.rewardItems.length, 0, '空 batch 列表时 rewardItems 应为空');
});

test('属性 11 边界：全部战败时 exp/silver 为零', () => {
  const batches: BatchSummary[] = [
    { result: 'defender_win', expGained: 0, silverGained: 0, itemsGained: [] },
    { result: 'defender_win', expGained: 0, silverGained: 0, itemsGained: [] },
    { result: 'draw', expGained: 0, silverGained: 0, itemsGained: [] },
  ];
  const summary = computeSessionSummary(batches);
