 ...
 *
 * 作用（做什么 / 不做什么）：
 * 1. 做什么：验证一字焚诀会统一走规范化、格式校验、敏感词校验与 prompt 语境构造。
 * 2. 做什么：锁定“留空视为未填写、仅 1 个中文字符合法、敏感词必须拦截”的共享口径。
 ...
 * 2. 敏感词检测必须复用现有共享服务，否则一字焚诀会和角色名、功法命名形成不同词库口径。
 */
...
test('validateTechniqueBurningWordPrompt: 合法单个中文字符应通过', () => {
  assert.deepEqual(validateTechniqueBurningWordPrompt(' 焰 '), {
    success: true,
    value: '焰',
  });
});

test('validateTechniqueBurningWordPrompt: 超过 1 个中文字符应被拦截', () => {
  const result = validateTechniqueBurningWordPrompt('焚诀');

  assert.equal(result.success, false);
  if (result.success) return;
  assert.equal(result.message, '一字焚诀最多 1 个中文字符');
});
...
test('validateTechniqueBurningWordPrompt: 非中文字符应被拦截', () => {
  ...
  assert.equal(result.success, false);
  if (result.success) return;
  assert.equal(result.message, '一字焚诀只能包含中文字符');
});
...
test('buildTechniqueBurningWordPromptContext: 合法单字应生成稳定 extraContext', () => {
  assert.deepEqual(buildTechniqueBurningWordPromptContext('炎'), {
    techniqueBurningWordPrompt: '炎',
  });
