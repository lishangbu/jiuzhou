 ...
 *
 * 作用（做什么 / 不做什么）：
 * 1. 做什么：集中定义常规 tick 是否还能继续补派任务的预算规则，避免 runner 主流程把“时间预算 / 单轮派发上限”判断散落在循环内部。
 * 2. 做什么：为常规调度与测试提供同一份纯函数策略，确保线上行为与回归断言一致。
 * 3. 不做什么：不读取任务队列、不操作 Promise，也不修改任务状态。
 *
 * 输入 / 输出：
 * - 输入：当前是否强制 drain 全量、tick 已运行时长、已派发任务数、时间预算和最大派发数。
 ...
 * 2. 时间预算和派发上限任一达到阈值都必须停止补派；否则 backlog 稍大时总耗时仍会持续飙高。
 */
...
export const shouldContinueOnlineBattleSettlementDispatch = (params: {
  ...
  return params.elapsedMs < params.timeBudgetMs;
};
