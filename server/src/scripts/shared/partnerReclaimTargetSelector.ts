 ...
 *
 * 作用（做什么 / 不做什么）：
 * 1. 做什么：集中解析 `--partner-ids` 参数，并把脚本目标收口成“按底模”或“按伙伴 ID”两种互斥模式。
 * 2. 做什么：统一生成目标摘要文案，避免 CLI、报告摘要和测试各自重复拼接模式说明。
 * 3. 不做什么：不查询数据库、不执行伙伴回收，也不决定伙伴返还内容。
 *
 * 输入/输出：
 * - 输入：原始 `--partner-ids` 字符串、底模列表、已解析的伙伴 ID 列表、未命中目标列表。
 * - 输出：去重后的伙伴 ID 数组、目标选择对象，以及报告摘要文案。
 *
 * 数据流/状态流：
 ...
 * 2. 一旦进入按 ID 模式，就必须完全忽略底模列表；这里不能再留“同时命中底模”的隐式分支。
 */
